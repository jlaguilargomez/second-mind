...
test('la reconexión segura guarda un snapshot local antes de reemplazar notas o ajustes', async () => {
  ...
  assert.match(composable, /function cloneForStorage\(value\)/)
  ...
  assert.match(composable, /notes: cloneForStorage\(notes\.value\.map\(\(note\) => normalizeNote\(note\)\)\)/)
  assert.match(composable, /settings: cloneForStorage\(normalizeWorkspaceSettings\(workspaceSettings\.value\)\)/)
  assert.match(composable, /recoverySnapshots\.value = cloneForStorage\(\[snapshot, \.\.\.existingSnapshots\]\.slice\(0, 3\)\)/)
  assert.match(composable, /await repository\.setSetting\(WORKSPACE_RECOVERY_KEY, \{/)
  assert.match(composable, /snapshots: cloneForStorage\(recoverySnapshots\.value\)/)
